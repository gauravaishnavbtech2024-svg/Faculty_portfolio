    ...
    const handleScroll = () => {
      ...
      for (let i = sectionIds.length - 1; i >= 0; i--) {
        ...
        if (el) {
          ...
          // If section top is within top 40% of viewport or scrolled into
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5">
            {data.links?.scholar && (
              <a
                ...
                className="w-8 h-8 rounded-full bg-white/10 hover:bg-white text-white hover:text-[#2547d0] flex items-center justify-center transition-all duration-150 shadow-sm"
              >
            ...
            {data.links?.orcid && (
              <a
                href={data.links.orcid}
                target="_blank"
                rel="noopener noreferrer"
                title="ORCID ID"
                className="w-8 h-8 rounded-full bg-white/10 hover:bg-white text-white hover:text-[#2547d0] flex items-center justify-center transition-all duration-150 shadow-sm text-xs font-bold"
              >
                iD
              </a>
            )}

            {data.links?.researchgate && (
              <a
                href={data.links.researchgate}
                target="_blank"
                rel="noopener noreferrer"
            ...
            {data.links?.website && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                title="Personal Homepage / Lab Website"
                className="w-8 h-8 rounded-full bg-white/10 hover:bg-white text-white hover:text-[#2547d0] flex items-center justify-center transition-all duration-150 shadow-sm"
              >
            ...
            {data.visible?.email && data.contact?.email && (
              <a
                href={`mailto:${data.contact.email}`}
                title={`Email ${data.name || 'Faculty'}`}
                className="w-8 h-8 rounded-full bg-white/10 hover:bg-white text-white hover:text-[#2547d0] flex items-center justify-center transition-all duration-150 shadow-sm"
              >
                <svg className="w-3.5 h-3.5 fill-none stroke-current stroke-2" viewBox="0 0 24 24">
